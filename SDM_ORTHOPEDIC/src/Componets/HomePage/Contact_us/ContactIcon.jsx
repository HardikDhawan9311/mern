    ...
    <div className="fixed bottom-0 right-0 m-6 z-50 md:hidden">
      <div className={`formbold-form-wrapper mx-auto w-full max-w-[550px] rounded-lg border border-[#e0e0e0] bg-white ${isOpen ? '' : 'hidden'}`}>
        ...
        <form action="https://formbold.com/s/FORM_ID" method="POST" className="py-6 px-9">
          <div className="mb-5">
            <label htmlFor="name" className="mb-3 block text-base font-medium text-[#07074D]">Enter Your Name</label>
            <input type="text" name="name" id="name" placeholder="Enter your name" className="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
          </div>
          <div className="mb-5">
          ...
          <div className="mb-5">
            ...
            <textarea rows="4" name="message" id="message" placeholder="Explain your queries" className="w-full resize-none rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"></textarea>
          </div>
          <div>
            <button className="hover:shadow-form w-full rounded-md bg-cyan-600 py-3 px-8 text-center text-base font-semibold text-white outline-none">Start Chat</button>
          </div>
        </form>
      </div>
      <div className="flex items-center justify-end mt-4">
        <button className="flex h-[70px] w-[70px] items-center justify-center rounded-full bg-cyan-600 text-white" onClick={chatboxToggleHandler}>
          {isOpen ? (
